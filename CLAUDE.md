...
- 加工判定スコア：4指標（背景の歪み・テクスチャの均一化・光源・影の不整合・切り貼り・合成の痕跡）の平均。ELAはスコア計算対象外・参考表示のみ
...
旧4指標（ノイズ整合性・DCT・PRNU・ノイズCoV）はフォレンジック系で実用性が低かったため全削除。実際の加工（盛り画像・合成・スタンプ貼り）を検出する新4指標に完全置き換え済み。
