...
- `backend/services/texture_service.py` — 局所テクスチャ分散マップ。不自然に滑らかな領域を赤でハイライト（AI生成画像に特有）。可視化は分散閾値80。スコアは分散300超の高テクスチャブロック割合で補正（カメラ写真は必ず存在・AI画像はほぼない）
- `backend/services/noise_service.py` — ノイズレベル解析。ノイズが少なすぎる領域を赤でハイライト（AI生成画像に特有）。スコアはノイズ多様性（CV）で補正・係数0.8（均一に低ノイズ＝AI・不均一＝本物写真）
- `backend/services/noise_consistency_service.py` — ノイズ整合性解析。不整合ブロックのみ赤でハイライト。偏差閾値0.5・係数2.0（加工検出感度を上げるよう調整）。詳細は判定テキスト＋見方のみ表示（割合数値は非表示）
- `backend/services/dct_splicing_service.py` — DCTスプライシング検出。異常ブロックのみ赤でハイライト。z閾値2.0・係数3.0（加工検出感度を上げるよう調整）。詳細は判定テキスト＋見方のみ表示（割合数値は非表示）
...
- `backend/services/manipulation_service.py` — ノイズCoV検出。外れ値ブロックを赤でハイライト。外れ値閾値3σ・CoV重み0.3・外れ値重み0.7・正規化0.03（加工検出感度を上げるよう調整）。詳細は判定テキスト＋解説のみ表示（生数値は非表示）
