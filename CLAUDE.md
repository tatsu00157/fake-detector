...
- `backend/services/c2pa_service.py` — C2PA署名検出。c2paライブラリ＋XMPメタデータでAI生成ツールの署名を確認。AI判定タブに表示（スコア計算には含まない）
...
- `backend/services/prnu_service.py` — ノイズ残差マップ（最高精度の加工検出）。全ユーザーに実行
- `backend/services/manipulation_service.py` — ノイズCoV検出。外れ値ブロックを赤でハイライトしたヒートマップを生成。加工スコアに使用
