...
- `backend/services/manipulation_service.py` — ノイズCoV検出。外れ値ブロックを赤でハイライト。外れ値閾値3σ・CoV重み0.3・外れ値重み0.7・正規化0.1（元のパラメータ。AI画像の誤検出を防ぐ）。詳細は判定テキスト＋解説のみ表示（生数値は非表示）
...
- AI判定スコア：3指標（Exif・テクスチャ・ノイズレベル）の平均。Exifはメーカー＋機種あり→0・どちらか一方→0.15・露光/ISO/焦点距離のみ→0.25・汎用メタデータのみ→0.5・メタデータなし→0.8・AI署名→0.95
...
### 現在の加工検出の問題点
現在の4指標（ノイズ整合性・DCT・PRNU・ノイズCoV）は「**カメラ写真に別素材を貼り付けた痕跡**」を検出するフォレンジック手法。何十回も調整を試みたが機能しなかった根本原因は**前提条件が間違っていたから**。

### 検出したい加工の種類
- **盛り画像** — 顔・体型を細くする・目を大きくするなどの美容アプリ系変形
- **合成** — 人物や背景を別素材と組み合わせた画像
- **ホワイトバランス・影の不整合** — 光源や影の方向が画像内で矛盾している
- **肌の過剰平滑化** — 美容アプリの美肌処理による高周波成分の消失
- **骨格・体型変形** — Liquifyツール等による背景の歪み

### 次回実装する新しい加工検出指標（OpenCV+NumPyのみ・MLモデル不要・VPS RAM 1GBで動作可能）
1. **背景歪み検出** — Liquifyで顔・体を変形すると周囲の背景に歪みが出る
2. **肌の過剰平滑化検出** — 美容アプリの美肌処理は高周波成分が消える（スキンマスクは色ベースで検出）
3. **照明・影の方向不整合** — 勾配マップで画像内の光源方向の矛盾を検出
4. **合成境界検出** — ELA強化版＋エッジの不自然さで合成境界を検出
...
| **人為的加工検出** | 盛り画像・美容アプリ加工・合成・ホワイトバランス・影の不整合など |
