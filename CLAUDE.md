...
- `src/pages/Pricing.tsx` — 料金プランページ（無料・プレミアム980円/月）。プランカード＋機能比較表。Stripe Checkoutへリダイレクト。ログイン中はSubscriptionsテーブルを参照して現在のプランを表示
...
- 解析履歴の保存
- PDFレポート出力
- `src/pages/Pricing.tsx` の機能比較表を最新の無料/有料構成に修正（ノイズ残差マップを無料→プレミアム限定に変更、ノイズ整合性解析・DCTスプライシング検出を無料機能として追加）
