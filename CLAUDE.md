...
- `src/components/ResultsDashboard.tsx` — AI生成検出・加工検出をタブUI（メタデータ・テクスチャ・ノイズ等）で切り替え表示。セクションごとの総合スコアバッジ。タブに判定カラードット表示
...
- `backend/services/noise_consistency_service.py` — ノイズ整合性解析。不整合ブロックのみ赤でハイライト。偏差閾値0.7・係数1.5（AI画像の誤検出を減らすよう調整済み）
- `backend/services/dct_splicing_service.py` — DCTスプライシング検出。異常ブロックのみ赤でハイライト。z閾値2.5・係数2.0（AI画像の誤検出を減らすよう調整済み）
- `backend/services/prnu_service.py` — ノイズ残差マップ（最高精度の加工検出）。全ユーザーに実行。anomaly_ratioをベースにconcentration(p95-median)で補正するスコア計算。「一部突出」(切り貼り)と「全体的に高い」(AI)を区別
- `backend/services/manipulation_service.py` — ノイズCoV検出。外れ値ブロックを赤でハイライト。外れ値閾値3σ・CoV重み0.3・外れ値重み0.7（AI画像の誤検出を減らすよう調整済み）
